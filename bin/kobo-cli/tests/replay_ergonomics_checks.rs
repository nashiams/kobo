...
fn explicit_opaque_boundary_downgrades_to_partial_with_replay_grade_and_ledger() {
    let project = TestProject::new("runtime-opaque-boundary-ledger");
...
fn lifecycle_observation_reports_inferred_obligations_from_semantics_not_text_bait() {
    let project = TestProject::new("runtime-lifecycle-inference");
...
"#,
    ...
    assert!(
        ...
        "public lifecycle facts should not expose internal-staging template versions: {obligation}"
    );
...
fn normal_code_lifecycle_templates_are_inferred_without_manual_must_call_attrs() {
    let project = TestProject::new("runtime-normal-lifecycle-inference");
    let file = project.main_file(
        r#"
struct Queue {}
struct Delivery {}
