...
#[test]
fn public_runtime_fixture_acceptance_flows_are_real_files() {
...
fn deep_scheduler_finds_interleaving_that_quick_does_not() {
    let project = TestProject::new("runtime-schedule-sensitive");
...
fn storage_commit_order_controls_durable_loss() {
    let project = TestProject::new("runtime-storage-state-machine");
...
fn network_delivery_state_changes_with_drop_delay_reorder() {
    let project = TestProject::new("runtime-network-state-machine");
...
fn public_backend_commands_report_stable_execution_truth() {
    let project = TestProject::new("runtime-backend-truth");
    ...
    assert!(
        ...
        "public backend output should not expose internal-staging version fields: {value}"
    );
    ...
    assert!(
        ...
        "slice 09 Loom support must execute generated user Rust inside Loom, not a lowered scenario model: {value}"
    );
    assert!(
        !output.combined().contains("runtime"),
        "public backend output must not report stale runtime-only execution status"
    );
}
...
fn loom_backend_adapter_is_a_real_workspace_dependency() {
    ...
    assert_contains(
        &lock,
        "name = \"loom\"",
        "runtime slice 09 must use a real Loom dependency, not only a backend label",
    );
}

#[test]
fn loom_backend_digest_discloses_generated_scenario_execution() {
    let project = TestProject::new("runtime-loom-boundary");
    let output = run_kobo(
        &[
            s("test"),
            s("--sim"),
    ...
    assert_eq!(
        ...
        "runtime sync backend must bind generated user Rust into Loom execution"
    );
}
...
fn exact_witness_events_have_stable_ids() {
    let project = TestProject::new("runtime-event-ids");
    let file = project.main_file(
        r#"
...
fn replay_rejects_shrink_that_removes_semantic_event() {
    let project = TestProject::new("runtime-shrink-necessary-event");
    let file = project.main_file(
        r#"
#[kobo::must_call(reply | reject | cancel)]
struct ReplyToken {}
...
fn witness_records_function_summaries_and_operation_coverage() {
    let project = TestProject::new("runtime-summary-coverage");
    let file = project.main_file(
        r#"
#[kobo::must_call(ack | nack)]
struct Delivery {}
...
fn deep_shrinker_removes_replay_irrelevant_network_ordering() {
    let project = TestProject::new("runtime-shrink-network-ordering");
    let file = project.main_file(
        r#"
#[kobo::must_call(reply | reject | cancel)]
struct ReplyToken {}
...
fn select_boundary_refuses_exact_until_modeled() {
    let project = TestProject::new("runtime-select-boundary");
    let output = run_kobo(
        &[
            s("test"),
            s("--sim"),
...
fn select_coverage_is_compiler_owned() {
    let text_only_project = TestProject::new("runtime-select-text-only");
...
"#,
    ...
    let parsed_project = TestProject::new("runtime-select-parsed");
