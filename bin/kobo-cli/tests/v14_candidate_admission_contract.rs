...
fn complete_candidate_attr() -> &'static str {
    ...
    diagnostic_snapshot = "v14_candidate_msrv_snapshot",
...
fn replay_candidate_attr() -> &'static str {
    ...
    strict = "true",
    whole_ecosystem = "false",
    diagnostic_snapshot = "v14_candidate_adapter_snapshot",
    replay_related = "true"
)]"#
}
...
fn candidate_requires_inspect_visibility() {
    let project = TestProject::new("v14-candidate-inspect");
...
fn candidate_requires_manual_rust_equivalent() {
    let project = TestProject::new("v14-candidate-manual");
...
fn candidate_requires_strict_compatibility() {
    let project = TestProject::new("v14-candidate-strict");
    let source = candidate_source(
    ...
    strict = "false",
    whole_ecosystem = "false",
    diagnostic_snapshot = "v14_candidate_proc_macro_snapshot",
    replay_related = "false"
)]"#,
    );
...
fn candidate_rejects_whole_ecosystem_modeling_requirement() {
    let project = TestProject::new("v14-candidate-ecosystem");
    let source = candidate_source(
...
fn candidate_requires_diagnostic_snapshots() {
    let project = TestProject::new("v14-candidate-diagnostics");
    let source = candidate_source(
...
fn candidate_requires_replay_grade_and_adapter_confidence_when_replay_related() {
    let project = TestProject::new("v14-candidate-replay");
