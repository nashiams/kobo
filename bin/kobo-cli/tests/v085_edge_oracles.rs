...
fn phase_00_edge_unknown_and_case_variant_explain_are_structured() {
    ...
    assert_failure(&unknown, "phase 00 unknown diagnostic explain");
    assert_has(
        &lowercase.combined(),
        "K0107",
        "phase 00 explain should normalize user-entered diagnostic codes",
    );
    assert_has(
        &lowercase.combined(),
        "slug: unmodeled-external-boundary",
        "phase 00 normalized explain must still show registry slug",
    );
    assert_has(
        &unknown.combined(),
        "unknown diagnostic code",
        "phase 00 unknown explain must be a structured user-facing error",
    );
    assert_has(
        &unknown.combined(),
        "nearest",
        "phase 00 unknown explain should suggest nearby registered codes",
    );
}

#[test]
fn phase_01_edge_multiple_parse_errors_keep_distinct_spans_and_good_analysis() {
...
"#,
    ...
    assert!(
        ...
        "phase 01 must report multiple independent K011x recovery diagnostics, got {codes:?}\n{}",
        output.combined()
    );
    assert_ne!(
        recovery_lines[0], recovery_lines[1],
        "phase 01 recovery diagnostics must preserve distinct source spans"
    );
    assert!(
        codes.iter().any(|code| code == "K0001"),
        "phase 01 recovery must still analyze the non-poisoned good function, got {codes:?}"
    );
}

#[test]
fn phase_02_edge_lsp_handles_no_project_and_exposes_code_action_data() {
    let fixture = Fixture::file(
        "v085-edge-phase02",
        "no_project.kobo",
        r#"
...
"#,
    ...
    assert_success(&output, "phase 02 LSP no-project diagnostic export");
    let text = output.combined();
    assert_has(
        &text,
        "\"code\":\"K0001\"",
        "phase 02 LSP must publish normal compiler diagnostics",
    );
    assert_has(
        &text,
        "\"codeDescription\"",
        "phase 02 LSP diagnostic must include explain navigation",
    );
    assert_has(
        &text,
        "\"data\"",
        "phase 02 LSP diagnostic must include Kobo-specific machine data",
    );
}

#[test]
fn phase_03_edge_malformed_must_call_is_registry_diagnostic() {
    let empty = Fixture::file(
        "v085-edge-phase03-empty",
...
"#,
    ...
    for fixture in [&empty, &malformed] {
        ...
        assert_failure(&output, "phase 03 malformed must_call should fail");
        let text = output.combined();
        assert_has(
            &text,
            "must_call",
            "phase 03 malformed metadata diagnostic must name the attribute",
        );
        assert_has(
            &text,
            "\"slug\"",
            "phase 03 malformed metadata diagnostic must use registry identity",
        );
        assert_lacks(
            &text,
            "panicked",
            "phase 03 malformed metadata must not crash the compiler",
        );
    }
}

#[test]
fn phase_04_edge_escape_and_suppression_reason_are_not_same_diagnostic() {
    let escape = Fixture::file(
        "v085-edge-phase04-escape",
        "escape.kobo",
        r#"
#[kobo::must_call(commit | rollback)]
...
"#,
    ...
    assert_has(
        &escape_out.combined(),
        "K0101",
        "phase 04 escaping obligation must use K0101, not generic K0100",
    );
    assert_has(
        &no_reason_out.combined(),
        "reason",
        "phase 04 suppression without reason must be rejected",
    );
    assert_has(
        &reason_out.combined(),
        "K0108",
        "phase 04 reasoned suppression must be recorded as K0108",
    );
    assert_ne!(
        escape_out.combined(),
        reason_out.combined(),
        "phase 04 escape and suppression reports cannot be one hardcoded message"
    );
}

#[test]
fn phase_05_edge_scout_is_stable_and_does_not_modify_source() {
    let fixture = Fixture::file(
        "v085-edge-phase05",
        "scout_stable.kobo",
        r#"
...
"#,
    ...
    assert_success(&first, "phase 05 first scout run");
    assert_success(&second, "phase 05 second scout run");
    assert_eq!(
        before,
        fixture.read(),
        "phase 05 scout must not modify source files"
    );
    assert_eq!(
        first.combined(),
        second.combined(),
        "phase 05 scout output must be stable for identical input"
    );
    assert_has(
        &first.combined(),
        "next_command",
        "phase 05 scout output must include an actionable next command",
    );
}

#[test]
fn phase_06_edge_duplicate_raw_sources_are_grouped_but_counted() {
    let fixture = Fixture::file(
        "v085-edge-phase06",
        "duplicate_raw.kobo",
        r#"
...
"#,
    ...
    assert!(
        ...
        "phase 06 duplicate raw sources must still count each nondeterminism class, got {codes:?}\n{text}"
    );
    assert_has(
        &text,
        "occurrences",
        "phase 06 duplicate raw source groups must expose machine-readable counts",
    );
    assert_has(
        &text,
        "SystemTime::now",
        "phase 06 scanner must preserve source-specific raw operation names",
    );
}

#[test]
fn phase_07_edge_kwit_missing_span_fails_unknown_fields_survive() {
    let missing_span = Fixture::file(
        "v085-edge-phase07-missing",
        "missing_span.kwit",
        r#"{
  "schema_version": 0,
...
"#,
    ...
    assert_failure(&missing_out, "phase 07 witness missing span");
    assert_success(&unknown_out, "phase 07 witness unknown-field roundtrip");
    assert_has(
        &missing_out.combined(),
        "span",
        "phase 07 invalid witness must name the missing schema field",
    );
    assert_has(
        &unknown_out.combined(),
        "\"future\"",
        "phase 07 roundtrip must preserve forward-compatible unknown fields",
    );
}

#[test]
fn phase_08_edge_malformed_scenario_attribute_and_removed_variant() {
    let malformed = Fixture::file(
        "v085-edge-phase08-malformed",
...
"#,
    ...
    assert_failure(&malformed_out, "phase 08 malformed scenario attribute");
    assert_success(&removed_out, "phase 08 removed scenario metadata inspect");
    assert_has(
        &malformed_out.combined(),
        "scenario",
        "phase 08 malformed scenario diagnostic must name the attribute",
    );
    assert_lacks(
        &removed_out.combined(),
        "scenario",
        "phase 08 removing the attribute must remove scenario metadata",
    );
}

#[test]
fn phase_09_edge_boundary_policy_matrix_is_source_sensitive() {
    let modeled = Fixture::file(
        "v085-edge-phase09-model",
        "modeled.kobo",
        r#"
...
"#,
    ...
    assert_has(
        ...
        "model",
        "phase 09 model policy must render explicitly",
    );
    assert_has(
        &debt_out.combined(),
        "debt",
        "phase 09 debt policy must render explicitly",
    );
    assert_ne!(
        modeled_out.combined(),
        debt_out.combined(),
        "phase 09 changing boundary policy must change output"
    );
    assert_has(
        &missing_out.combined(),
        "reason",
        "phase 09 boundary policy without reason must be rejected",
    );
}
...
fn phase_10_edge_backend_registry_lists_backends_without_executing_them() {
    ...
    assert_lacks(
        &text,
        "executing",
        "phase 10 backend registry listing must be metadata-only",
    );
}

#[test]
fn phase_11_edge_suggestions_are_capped_and_absent_for_quiet_code() {
    let noisy = Fixture::file(
        "v085-edge-phase11-noisy",
        "noisy.kobo",
        r#"
#[kobo::must_call(commit | rollback)]
struct Transaction { id: u64 }
...
"#,
    ...
    assert_has(
        &noisy_text,
        "confidence",
        "phase 11 noisy fixture must label suggestion confidence",
    );
    assert!(
        suggestion_count <= 3,
        "phase 11 suggestions must be capped, got {suggestion_count} suggestions in:\n{noisy_text}"
    );
    assert_lacks(
        &quiet_out.combined(),
        "suggestions",
        "phase 11 quiet code must not receive generic suggestions",
    );
}

#[test]
fn phase_12_edge_budgeted_human_output_keeps_full_machine_diagnostics() {
    let fixture = Fixture::file(
        "v085-edge-phase12",
        "budget_machine.kobo",
        r#"
...
"#,
    ...
    assert_has(
        &human.combined(),
        "grouped",
        "phase 12 human output should group offscreen/low-priority diagnostics",
    );
    let machine_codes = json_codes(&machine);
    assert!(
        code_count(&machine_codes, "K0001") >= 3,
        "phase 12 machine output must retain budgeted diagnostics, got {machine_codes:?}\n{}",
        machine.combined()
    );
}

#[test]
fn phase_13_edge_k0061_card_is_short_but_explain_is_detailed() {
    let fixture = Fixture::file(
        "v085-edge-phase13",
        "k0061_edge.kobo",
        r#"
...
"#,
    ...
    assert_has(
        &card.combined(),
        "K0061",
        "phase 13 fixture must render the rich async shared-state card",
    );
    assert_has(
        &card.combined(),
        "kobo explain K0061",
        "phase 13 short card must point to detailed explain text",
    );
    assert_has(
        &explain.combined(),
        "LocalSet",
        "phase 13 explain must include full detailed remediation paths",
    );
    assert!(
        card.combined().lines().count() < explain.combined().lines().count(),
        "phase 13 CLI card should stay shorter than detailed explain output"
    );
}

#[test]
fn phase_14_edge_fix_refuses_placeholder_and_overlapping_edits() {
    let fixture = Fixture::file(
        "v085-edge-phase14",
        "overlap.kobo",
        r#"
...
"#,
    ...
    assert_success(&dry_run, "phase 14 overlapping fix dry-run");
    assert_failure(&apply, "phase 14 overlapping fix apply");
    assert_has(
        &dry_run.combined(),
        "overlap",
        "phase 14 dry-run must explain overlapping edit refusal",
    );
    assert_has(
        &dry_run.combined(),
        "HasPlaceholders",
        "phase 14 dry-run must expose placeholder edits as non-applyable",
    );
    assert_eq!(
        before,
        fixture.read(),
        "phase 14 refused fixes must not modify the source"
    );
}

#[test]
fn phase_15_edge_invalid_field_capability_has_source_mapped_diagnostic() {
    let fixture = Fixture::file(
        "v085-edge-phase15",
        "invalid_field.kobo",
        r#"
...
"#,
    ...
    let output = run(&with_file(&["check", "--error-format=json"], &fixture.file));
    assert_failure(&output, "phase 15 invalid field capability");
    let text = output.combined();
    assert_has(
        &text,
        "missing",
        "phase 15 invalid field diagnostic must name the bad field",
    );
    assert_has(
        &text,
        "\"slug\"",
        "phase 15 invalid field diagnostic must be registry-backed",
    );
    assert_lacks(
        &text,
        "fn route(",
        "phase 15 invalid using syntax must not be lowered into fake Rust",
    );
}

#[test]
fn phase_16_edge_normal_project_builds_and_profile_outputs_differ() {
    let normal = Fixture::project(
        "v085-edge-phase16-normal",
        &[
            (
                "Cargo.toml",
                r#"[package]
...
"#,
    ...
    assert_success(
        &normal_build,
        "phase 16 normal crate without Kobo metadata must still build",
    );
    assert_success(&api_doctor, "phase 16 API doctor fixture");
    assert_success(&storage_doctor, "phase 16 storage doctor fixture");
    assert_has(
        &api_doctor.combined(),
        "api-service",
        "phase 16 API dependency shape must select API service profile hints",
    );
    assert_has(
        &storage_doctor.combined(),
        "storage-engine",
        "phase 16 storage dependency shape must select storage profile hints",
    );
    assert_ne!(
        api_doctor.combined(),
        storage_doctor.combined(),
        "phase 16 profile recommendations must be project-sensitive"
    );
}
