...
fn parse_query_is_cached_per_source_fingerprint() {
    let root = std::env::temp_dir().join("kobo-query-check");
...
#[test]
fn repeated_stage_queries_reuse_cached_results() {
    let root = std::env::temp_dir().join("kobo-query-stage-check");
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(&root).unwrap();
    let file = root.join("demo.kobo");
    fs::write(
...
fn source_content_change_invalidates_downstream_queries() {
    let root = std::env::temp_dir().join("kobo-query-source-invalidation-check");
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(&root).unwrap();
    let file = root.join("demo.kobo");
...
fn relevant_config_change_invalidates_affected_downstream_queries_without_reparsing() {
    let root = std::env::temp_dir().join("kobo-query-config-invalidation-check");
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(&root).unwrap();
    let file = root.join("demo.kobo");
    fs::write(
...
fn old_pipeline_api_and_query_api_produce_same_generated_source() {
    let root = std::env::temp_dir().join("kobo-query-compat-check");
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(&root).unwrap();
    let file = root.join("demo.kobo");
