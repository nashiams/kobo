...
"#,
    ...
    let dev = run_kobo(
        &[s("build"), s("--profile"), s("dev"), s("--emit-rust"), path_arg(&file)],
        &project.root,
    );
    ...
    assert_contains(&dev.combined(), "Box<dyn", "dev should use ergonomic boxed error shape");
    assert_contains(&checked.combined(), "enum", "checked should generate typed enum shape");
    assert_contains(&release.combined(), "explicit", "release should require explicit error policy evidence");
}
...
"#,
    ...
    for choice in ["model", "record", "stub", "outside", "opaque", "debt"] {
        assert_contains(&text, choice, "K0107 must expose every boundary policy choice");
    }
}
