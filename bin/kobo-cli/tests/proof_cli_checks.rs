...
fn proof_emit_writes_kproof_with_required_schema() {
    let (_project, _source_file, artifact_path) =
        emit_project("proof-proof-emit", "proof_emit_case");
...
fn proof_verify_accepts_valid_artifact() {
    let (project, _source_file, artifact_path) =
        emit_project("proof-proof-verify-valid", "proof_verify_valid_case");
...
fn proof_verify_rejects_tampered_artifact() {
    let (project, _source_file, artifact_path) =
        emit_project("proof-proof-verify-tampered", "proof_verify_tampered_case");
...
fn proof_verify_rejects_artifact_kind_path_mismatch() {
    let (project, _source_file, artifact_path) =
        emit_project("proof-proof-verify-kind-mismatch", "proof_verify_kind_case");
...
fn proof_verify_rejects_invalid_header_before_source_path_read() {
    let (project, _source_file, artifact_path) = emit_project(
        "proof-proof-verify-header-before-source",
...
fn proof_verify_rejects_unsupported_artifact_extension() {
    let (project, _source_file, artifact_path) = emit_project(
        "proof-proof-verify-unsupported-extension",
    ...
    assert!(
        ...
        "unsupported extension rejection should name the path check: {}",
        output.combined()
    );
}
...
fn proof_emit_rejects_unsupported_output_extension() {
    let project = TestProject::new("proof-proof-emit-unsupported-extension");
    ...
    assert!(
        output
            .combined()
            .contains("unsupported proof artifact path"),
        "unsupported output extension rejection should name the path check: {}",
        output.combined()
    );
    assert!(
...
fn check_emit_proof_runs_check_and_emits_artifact() {
    let project = TestProject::new("proof-check-emit-proof");
...
fn check_emit_proof_refuses_exact_over_disallowed_boundary() {
    let project = TestProject::new("proof-check-emit-proof-boundary");
...
fn proof_cli_json_output_is_machine_readable() {
    let (project, _source_file, artifact_path) =
        emit_project("proof-proof-json", "proof_json_case");
