...
fn strict_errors_on_undischarged_must_call_path() {
    let project = TestProject::new("model-strict-undischarged");
...
fn check_release_emits_strict_liveness_compile_error() {
    let project = TestProject::new("model-strict-check-path");
...
fn check_release_rejects_unproven_helper_transfer() {
    let project = TestProject::new("model-strict-check-unproven-helper");
    let file = project.main_file(&delivery_source(
        r#"
...
fn check_release_rejects_unresolved_obligation_across_await() {
    let project = TestProject::new("model-strict-check-await");
    project.write(
        "Cargo.toml",
        r#"[package]
name = "model_strict_check_await"
version = "0.1.0"
edition = "2021"
...
fn check_release_rejects_unsupported_obligation_container() {
    let project = TestProject::new("model-strict-check-unsupported-container");
    let file = project.main_file(&delivery_source(
        r#"
...
fn strict_accepts_discharge_return_transfer_escape_and_reasoned_suppression() {
    let project = TestProject::new("model-strict-accepted-forms");
    let source = delivery_source(
        r#"
...
fn question_mark_with_unresolved_obligation_fails() {
    let project = TestProject::new("model-strict-question");
    let source = delivery_source(
        r#"
...
fn panic_with_unresolved_obligation_fails() {
    let project = TestProject::new("model-strict-panic");
    let source = delivery_source(
        r#"
#[kobo::scenario(profile = "sync")]
fn panic_case() {
...
fn cancel_edge_without_handler_fails() {
    let project = TestProject::new("model-strict-cancel");
    let source = delivery_source(
        r#"
#[kobo::scenario(profile = "async")]
fn cancel_case() {
...
fn aliasing_and_shadowing_track_same_obligation_token() {
    let project = TestProject::new("model-strict-alias-shadow");
    let source = delivery_source(
        r#"
#[kobo::scenario(profile = "sync")]
...
fn conditional_discharge_does_not_hide_leaking_branch() {
    let project = TestProject::new("model-strict-conditional-branch");
    let source = delivery_source(
        r#"
...
fn match_discharge_does_not_hide_leaking_arm() {
    let project = TestProject::new("model-strict-match-branch");
    let source = delivery_source(
        r#"
fn runtime_flag(input: i32) -> bool { input > 0 }
...
fn helper_summary_proves_discharge_without_manual_annotation() {
    let project = TestProject::new("model-strict-helper");
    let source = delivery_source(
        r#"
...
fn helper_transfer_without_summary_proof_remains_unresolved() {
    let project = TestProject::new("model-strict-unproven-helper");
    let source = delivery_source(
        r#"
fn helper_ignores(token: Delivery) {
    let _kept = token;
...
fn unknown_recursion_scc_conservatively_escapes_or_fails() {
    let project = TestProject::new("model-strict-recursive-scc");
    let source = delivery_source(
        r#"
...
fn arc_mutex_delivery_fails_without_obligation_aware_wrapper_or_declaration() {
    let project = TestProject::new("model-strict-arc-mutex");
    let source = delivery_source(
        r#"
use std::sync::{Arc, Mutex};
...
fn arc_mutex_text_bait_does_not_create_container_failure() {
    let project = TestProject::new("model-strict-arc-mutex-text-bait");
    let source = delivery_source(
        r#"
#[kobo::scenario(profile = "sync")]
...
fn opaque_boundary_exit_with_unresolved_obligation_fails() {
    let project = TestProject::new("model-strict-opaque-exit");
    let source = delivery_source(
        r#"
...
fn mode_invariant_preserves_runtime_output_for_accepted_code() {
    let project = TestProject::new("model-strict-mode-invariant");
