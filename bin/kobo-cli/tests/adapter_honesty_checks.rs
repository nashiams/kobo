...
fn adapter_confidence_exact_modeled_sampled_metadata_only_are_serialized() {
    let project = TestProject::new("proof-adapter-confidence-all");
    let specs = [
        AdapterSpec {
            crate_name: "exactpay",
            confidence: "exact",
...
fn sampled_adapter_emits_probing_pass_not_proof() {
    let project = TestProject::new("proof-sampled-adapter");
...
fn metadata_only_adapter_cannot_claim_replayable_behavior() {
    let project = TestProject::new("proof-metadata-adapter");
    let specs = [AdapterSpec {
        crate_name: "metapay",
        confidence: "metadata-only",
        version: "1.0.0",
...
fn stale_adapter_downgrades_to_debt_or_not_replayable() {
    let project = TestProject::new("proof-stale-adapter");
    let specs = [AdapterSpec {
...
fn exact_replay_rejected_through_policy_gap() {
    let project = TestProject::new("proof-exact-policy-gap");
...
fn exact_replay_rejected_through_stub_boundary() {
    let project = TestProject::new("proof-exact-stub-boundary");
...
fn exact_replay_rejected_when_record_boundary_lacks_adapter_evidence() {
    let project = TestProject::new("proof-exact-unsupported-adapter");
...
fn replay_grade_and_adapter_confidence_visible_in_kwit_kproof_inspect_and_json() {
    let project = TestProject::new("proof-adapter-visibility");
    let specs = [AdapterSpec {
        crate_name: "exactpay",
        confidence: "exact",
        version: "1.0.0",
