//! RED oracle tests for the plan.
//!
//! These tests intentionally exercise public CLI behavior before the feature
//! work exists. They should fail until each slice is implemented through the
...
#[test]
fn slice_00_explain_and_json_share_registry_identity() {
    let boundary = run_kobo(&args(&["explain", "K0107", "--verbose"]));
    let suppression = run_kobo(&args(&["explain", "K0108", "--verbose"]));
    assert_success(&boundary, "slice 00 K0107 explain");
    assert_success(&suppression, "slice 00 K0108 explain");
    ...
    assert_contains_all(
        ...
        "slice 00 K0107 explain must come from the registry check",
    );
    assert_contains_all(
        &suppression_text,
        &[
        ...
        "slice 00 K0108 explain must come from the registry check",
    );
    assert_ne!(
        boundary_text, suppression_text,
        "slice 00 explain output must be code-specific, not a constant card"
    );

    let fixture = Fixture::new(
        "foundation-slice00-json",
        "bad.kobo",
        "fn main() { let x = }\n",
    );
    ...
    assert_json_identity(
        &diagnostics,
        "slice 00 diagnostic JSON must expose registry identity fields",
    );
}

#[test]
fn slice_01_recovery_and_query_foundation_are_content_sensitive() {
    let mixed = Fixture::new(
        "foundation-slice01-mixed",
        "mixed.kobo",
        r#"
fn broken() {
    let x =
...
"#,
    ...
    assert!(
        ...
        "slice 01 parser recovery must use non-conflicting K011x codes, got {mixed_codes:?}\n{}",
        mixed_out.combined()
    );
    assert!(
        !mixed_codes
        ...
        "slice 01 parser recovery must not consume foundation liveness/replay code space: {mixed_codes:?}"
    );
    assert!(
        mixed_codes.iter().any(|code| code == "K0001"),
        "slice 01 recovery must continue trustworthy analysis outside poisoned regions:\n{}",
        mixed_out.combined()
    );
    assert!(
        !fixed_codes.iter().any(|code| code == "K0001"),
        "slice 01 query/output must change when the non-poisoned source changes:\n{}",
        fixed_out.combined()
    );
    assert_json_identity(
        &diagnostic_json_values(&mixed_out),
        "slice 01 JSON diagnostics must include registry identity",
    );
}

#[test]
fn slice_02_lsp_payload_export_matches_cli_json() {
    let fixture = Fixture::new(
        "foundation-slice02-lsp",
        "lsp.kobo",
        r#"
fn main() {
...
"#,
    ...
    assert_failure(&cli, "slice 02 CLI fixture must produce a diagnostic");
    assert_success(&lsp, "slice 02 LSP diagnostic export");
    assert!(
        cli_codes.iter().any(|code| code == "K0001"),
        "slice 02 CLI baseline must include K0001, got {cli_codes:?}"
    );
    assert_eq!(
        cli_codes, lsp_codes,
        "slice 02 LSP payload must preserve the same diagnostic codes as CLI JSON"
    );
    for value in lsp_values {
        assert_contains_all(
            ...
            "slice 02 LSP diagnostics need editor-ready identity and explain action",
        );
    }
}

#[test]
fn slice_03_must_call_metadata_reaches_inspect_without_enforcement() {
    let commit_or_rollback = Fixture::new(
        "foundation-slice03-commit-rollback",
        "transaction.kobo",
        r#"
...
"#,
    ...
    let finish_or_cancel = Fixture::new(
        "foundation-slice03-finish-cancel",
        "operation.kobo",
        r#"
...
"#,
    ...
    assert_success(&tx, "slice 03 inspect must accept must_call metadata");
    assert_success(
        &op,
        "slice 03 inspect must accept alternate must_call obligations",
    );
    assert_contains_all(
        &tx.combined(),
        &["must_call", "commit", "rollback"],
        "slice 03 inspect output must expose obligation metadata",
    );
    assert_contains_all(
        &op.combined(),
        &["must_call", "finish", "cancel"],
        "slice 03 metadata must be source-sensitive",
    );
    assert_not_contains(
        &tx.combined(),
        "K0100",
        "slice 03 must not enforce unresolved obligations before slice 04",
    );
    assert_ne!(
        tx.combined(),
        op.combined(),
        "slice 03 metadata output cannot be hardcoded to one obligation set"
    );
}

#[test]
fn slice_04_liveness_debt_distinguishes_unresolved_commit_and_rollback() {
    let unresolved = Fixture::new(
        "foundation-slice04-unresolved",
        "unresolved.kobo",
        r#"
#[kobo::must_call(commit | rollback)]
struct Transaction {
...
"#,
    ...
    assert_success(
        &unresolved_out,
        "slice 04 debt --liveness unresolved fixture",
    );
    assert_success(&committed_out, "slice 04 debt --liveness resolved fixture");
    assert_contains_all(
        &unresolved_out.combined(),
        &["K0100", "Transaction", "commit", "rollback"],
        "slice 04 unresolved path must emit obligation-specific K0100",
    );
    assert_not_contains(
        &committed_out.combined(),
        "K0100",
        "slice 04 resolved rollback/commit paths must remove K0100",
    );
    assert_ne!(
        unresolved_out.combined(),
        committed_out.combined(),
        "slice 04 liveness report must be path-sensitive"
    );
}

#[test]
fn slice_05_sim_scout_ranking_changes_with_project_signals() {
    let backend = Fixture::new(
        "foundation-slice05-backend",
        "backend.kobo",
        r#"
...
"#,
    ...
    assert_success(&backend_out, "slice 05 scout backend fixture");
    assert_success(&simple_out, "slice 05 scout simple fixture");
    assert_contains_all(
        &backend_out.combined(),
        &["top_target", "retry_worker", "reasons", "next_command"],
        "slice 05 rich fixture must produce an actionable ranked target",
    );
    assert_contains_all(
        &simple_out.combined(),
        &["no high-value simulation target", "reasons"],
        "slice 05 simple fixture must not receive a fake high-value target",
    );
    assert_ne!(
        first_named_line(&backend_out.combined(), "\"top_target\""),
        first_named_line(&simple_out.combined(), "\"top_target\""),
        "slice 05 scout ranking must change with input signals"
    );
}

#[test]
fn slice_06_nondeterminism_scanner_counts_raw_sources_and_safe_variant() {
    let raw = Fixture::new(
        "foundation-slice06-raw",
        "raw.kobo",
        r#"
...
"#,
    ...
    assert!(
        ...
        "slice 06 raw scenario must report each nondeterminism class as K0102, got {raw_codes:?}\n{}",
        raw_out.combined()
    );
    assert_eq!(
...
#[test]
fn slice_07_kwit_replay_validates_schema_without_claiming_full_replay() {
    let valid = Fixture::new(
        "foundation-slice07-valid",
        "valid.kwit",
        r#"{
  "schema_version": 0,
...
"#,
    ...
    assert_success(&valid_out, "slice 07 replay valid witness");
    assert_failure(&invalid_out, "slice 07 replay invalid witness");
    assert_contains_all(
        &valid_out.combined(),
        &["metadata-only", "schema_version: 0", "src/main.kobo"],
        "slice 07 valid witness should print only schema/source metadata",
    );
    assert_not_contains(
        &valid_out.combined().to_lowercase(),
        "replayed successfully",
        "slice 07 must not claim deterministic replay completion",
    );
    assert_contains_all(
        &invalid_out.combined(),
        &["schema_version", "unsupported", "K"],
        "slice 07 invalid witness must fail with registry-backed diagnostic wording",
    );
}

#[test]
fn slice_08_scenario_metadata_is_indexed_without_hidden_simulation() {
    let scenario = Fixture::new(
        "foundation-slice08-scenario",
        "scenario.kobo",
        r#"
...
"#,
    ...
    assert_success(&scenario_out, "slice 08 scenario inspect");
    assert_success(&ordinary_out, "slice 08 ordinary inspect");
    assert_contains_all(
        &scenario_out.combined(),
        &["scenario", "payment_retry", "io", "retry"],
        "slice 08 scenario metadata must be visible through inspect",
    );
    assert_not_contains(
        &ordinary_out.combined(),
        "scenario",
        "slice 08 ordinary functions must not receive fake scenario metadata",
    );
    assert_not_contains(
        &scenario_out.combined().to_lowercase(),
        "simulated time",
        "slice 08 scenario attributes must not silently enable simulation",
    );
}

#[test]
fn slice_09_boundary_policy_keeps_normal_crates_compatible_and_replay_explicit() {
    let normal = Fixture::new(
        "foundation-slice09-normal",
        "normal.kobo",
        r#"
use reqwest::Client;
...
"#,
    ...
    let suppressed = Fixture::new(
        "foundation-slice09-suppressed",
        "suppressed.kobo",
        r#"
#[kobo::boundary(crate = "reqwest", policy = "outside", reason = "integration tested by service check")]
use reqwest::Client;
...
"#,
    ...
    assert_not_contains(
        &normal_out.combined(),
        "K0107",
        "slice 09 normal external crate use must not be blocked by replay policy",
    );
    assert_contains_all(
        &replay_out.combined(),
        &["K0107", "model", "record", "outside", "opaque", "debt"],
        "slice 09 replay-critical external boundary must emit explicit policy vocabulary",
    );
    assert_contains_all(
        &suppressed_out.combined(),
        &["K0108", "integration tested by service check"],
        "slice 09 reason-bearing boundary suppression must be recorded",
    );
}

#[test]
fn slice_10_backend_registry_recommendations_are_evidence_specific_and_metadata_only() {
    let async_boundary = Fixture::new(
        "foundation-slice10-async",
        "async_boundary.kobo",
        r#"
...
"#,
    ...
    assert_success(&async_out, "slice 10 async backend recommendation");
    assert_success(&property_out, "slice 10 property backend recommendation");
    assert_contains_all(
        &async_out.combined(),
        &["Loom", "Shuttle", "backend_fit"],
        "slice 10 async boundary should recommend concurrency DST backends",
    );
    assert_contains_all(
        &property_out.combined(),
        &["proptest", "backend_fit"],
        "slice 10 property fixture should recommend property testing backend",
    );
    assert_ne!(
        async_out.combined(),
        property_out.combined(),
        "slice 10 backend registry cannot return one fixed recommendation",
    );
    assert_not_contains(
        &async_out.combined(),
        "executing backend",
        "slice 10 registry must not execute DST backends in foundation",
    );
}

#[test]
fn slice_10_typescript_posture_controls_are_kobo_first_and_reserved() {
    let service = Fixture::new(
        "foundation-slice10-posture",
        "service.kobo",
        r#"
use axum::Router;
use reqwest::Client;
...
"#,
    ...
    assert_success(&why, "foundation posture scout --why");
    assert_contains_all(
        &why.combined(),
        &[
    ...
    assert_success(&sim, "foundation posture inspect --sim");
    assert_contains_all(
        &sim.combined(),
        &[
            "inspect --sim",
    ...
    assert_success(&harness, "foundation posture inspect --sim --harness");
    assert_contains_all(
        &harness.combined(),
        &[
            "inspect --sim --harness",
...
#[test]
fn slice_11_contextual_suggestions_are_pattern_specific_and_capped() {
    let async_send = Fixture::new(
        "foundation-slice11-async",
        "async_send.kobo",
        r#"
async fn worker() {
...
"#,
    ...
    assert_contains_all(
        ...
        "slice 11 async pattern needs async-specific suggestions",
    );
    assert_contains_all(
        &liveness_out.combined(),
        &["suggestions", "debt --liveness", "confidence"],
        "slice 11 repeated obligation pattern needs liveness suggestion",
    );
    assert_not_contains(
        &quiet_out.combined(),
        "suggestions",
        "slice 11 quiet fixture must not receive generic suggestions",
    );
    assert_ne!(
        async_out.combined(),
        liveness_out.combined(),
        "slice 11 suggestions must depend on the source pattern",
    );
}

#[test]
fn slice_12_warning_budget_prioritizes_visible_region_without_losing_machine_output() {
    let many = Fixture::new(
        "foundation-slice12-budget",
        "budget.kobo",
        r#"
fn first() {
    let a = String::from("a");
...
"#,
    ...
    assert_contains_all(
        ...
        "slice 12 visible region should survive the human output cap",
    );
    assert_contains_all(
        &top_out.combined(),
        &["first", "budgeted", "grouped", "machine_diagnostics"],
        "slice 12 changing visible region should change prioritized output",
    );
    assert_ne!(
        visible_out.combined(),
        top_out.combined(),
        "slice 12 warning budget cannot ignore visible-region input"
    );
}

#[test]
fn slice_13_k0061_rich_card_has_actionable_no_color_paths_without_internal_types() {
    let fixture = Fixture::new(
        "foundation-slice13-k0061",
        "k0061.kobo",
        r#"
async fn main() {
...
"#,
    ...
    assert_contains_all(
        ...
        "slice 13 K0061 card must show explicit source-mapped fix paths",
    );
    assert_not_contains(
        &text,
        "Owned<",
        "slice 13 diagnostics must not leak compiler-internal owned wrappers",
    );
    assert_not_contains(
        &text,
        "\u{1b}[",
        "slice 13 NO_COLOR fallback should be plain text",
    );
}

#[test]
fn slice_14_kobo_fix_applies_only_machine_applicable_idempotent_edits() {
    let fixture = Fixture::new(
        "foundation-slice14-fix",
        "fixable.kobo",
        r#"
fn main() {
    let name = String::from("alpha");
...
"#,
    ...
    assert_success(&dry_run, "slice 14 fix dry-run");
    assert_contains_all(
        &dry_run.combined(),
        &["MachineApplicable", "TextEdit", "clone"],
        "slice 14 dry-run must expose only safe machine-applicable edits",
    );
    assert_eq!(
        fixture.read_source(),
        original,
        "slice 14 dry-run must not modify the fixture"
    );

    let apply = run_kobo(&args_with_file(&["fix", "--apply"], &fixture.file));
    assert_success(&apply, "slice 14 fix apply");
    let after_apply = fixture.read_source();
    assert_ne!(
        after_apply, original,
        "slice 14 apply must change the fixable source"
    );
    let second_apply = run_kobo(&args_with_file(&["fix", "--apply"], &fixture.file));
    assert_success(&second_apply, "slice 14 second fix apply");
    assert_eq!(
        fixture.read_source(),
        after_apply,
        "slice 14 apply must be idempotent after the first safe edit"
    );
}

#[test]
fn slice_15_field_capability_views_lower_to_real_field_borrows() {
    let header_only = Fixture::new(
        "foundation-slice15-header",
        "header.kobo",
        r#"
struct Request {
    header: String,
...
"#,
    ...
    let header_and_body = Fixture::new(
        "foundation-slice15-header-body",
        "header_body.kobo",
        r#"
struct Request {
    header: String,
...
"#,
    ...
    assert_success(&header_out, "slice 15 header-only view inspect");
    assert_success(&both_out, "slice 15 header-body view inspect");
    assert_contains_all(
        &header_out.combined(),
        &["&req.header", "using"],
        "slice 15 generated Rust must reflect the field capability list",
    );
    assert_contains_all(
        &both_out.combined(),
        &["&req.header", "&req.body"],
        "slice 15 changing field list must change lowered Rust",
    );
    assert_ne!(
        header_out.combined(),
        both_out.combined(),
        "slice 15 lowering cannot ignore the using field list"
    );
    assert_contains_all(
        &duplicate_out.combined(),
        &["K", "duplicate", "header"],
        "slice 15 duplicate field capability must be a registry diagnostic",
    );
}

#[test]
fn slice_16_doctor_profiles_and_trait_defaults_are_project_sensitive() {
    let api = Fixture::project(
        "foundation-slice16-api",
        &[
            (
                "Cargo.toml",
                r#"[package]
...
"#,
    ...
    assert_success(&api_doctor, "slice 16 api doctor");
    assert_success(&game_doctor, "slice 16 game doctor");
    assert_contains_all(
        &api_doctor.combined(),
        &["api-service", "proc_macro", "feature", "msrv"],
        "slice 16 doctor must emit API-service dependency hints",
    );
    assert_contains_all(
        &game_doctor.combined(),
        &["game-server-core", "feature", "msrv"],
        "slice 16 doctor must emit game-server dependency hints",
    );
    assert_ne!(
        api_doctor.combined(),
        game_doctor.combined(),
        "slice 16 doctor cannot print one fixed project report"
    );
    ...
    assert_success(&api_inspect, "slice 16 api trait default inspect");
    assert_success(&game_inspect, "slice 16 game generic inspect");
    assert_contains_all(
        &api_inspect.combined(),
        &["Box<dyn Store>", "thin facade"],
        "slice 16 script/checked default should prefer trait-object facade",
    );
    assert_contains_all(
        &game_inspect.combined(),
        &["fn run_system<T: System>", "generic facade"],
        "slice 16 generic annotation/profile should opt into monomorphization",
    );
}
