...
fn sync_backend_adapter_writes_replay_token_and_stays_out_of_user_source() {
    let project = TestProject::new("runtime-backend-adapter");
...
fn ecosystem_backends_distinguish_native_execution_from_reserved_pins() {
    let project = TestProject::new("runtime-backend-registry-executable");
    let output = run_kobo(&[s("sim"), s("backends"), s("--json")], &project.root);
    assert_success(&output, "backend registry should render");
    assert_not_contains(
        &output.stdout,
        &["v", "0."].concat(),
        "backend registry should not expose internal-staging version wording",
    );
    assert_not_contains(
        &output.stdout,
        &["executes_in_v", "10"].concat(),
    ...
    assert!(
        ...
        "backend registry should not publish plan version field: {value}"
    );
...
fn async_witness_records_execution_backend_separately_from_reserved_fit() {
    let project = TestProject::new("runtime-async-execution-backend");
    let file = project.main_file(
        r#"
...
fn ecosystem_backend_registry_does_not_claim_full_external_crate_exploration() {
    let project = TestProject::new("runtime-backend-registry-scope");
    let output = run_kobo(&[s("sim"), s("backends"), s("--json")], &project.root);
    assert_success(&output, "backend registry should render");
    let value: Value = serde_json::from_str(&output.stdout).expect("backend JSON should parse");
    let backends = value["backends"]
...
fn expert_loom_backend_flags_drive_native_scheduler_output_without_source_imports() {
    let project = TestProject::new("runtime-expert-backend-flags");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "sync")]
...
fn default_sim_success_output_hides_backend_choices() {
    let project = TestProject::new("runtime-default-output-kobo-first");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "sync")]
...
fn show_backend_choices_opts_into_success_backend_details() {
    let project = TestProject::new("runtime-show-backend-choices");
    project.write(
        "Kobo.toml",
        r#"[sim]
...
fn unsupported_native_backend_scheduler_reports_explicit_debt_path() {
    let project = TestProject::new("runtime-unsupported-native-backend-scheduler");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "async")]
...
fn unsupported_native_backend_pin_reports_explicit_debt_path_without_scheduler() {
    let project = TestProject::new("runtime-unsupported-native-backend-pin");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "async")]
...
fn unsupported_backend_scheduler_combo_reports_explicit_debt_path() {
    let project = TestProject::new("runtime-unsupported-backend-knob");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "sync")]
fn expert_sync_route() {
...
fn inspect_harness_rejects_unsupported_backend_pin() {
    let project = TestProject::new("runtime-inspect-backend-pin");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "async")]
...
fn inspect_harness_reports_generated_manifest_and_source_path() {
    let project = TestProject::new("runtime-inspect-generated-harness");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "async")]
...
fn configured_reserved_backend_scheduler_reports_scenario_debt() {
    let project = TestProject::new("runtime-reserved-backend-config-debt");
    project.write(
        "Kobo.toml",
...
fn inspect_harness_configured_reserved_backend_reports_scenario_debt() {
    let project = TestProject::new("runtime-inspect-reserved-backend-config-debt");
    project.write(
        "Kobo.toml",
        r#"[sim.backend.shuttle]
enabled = true
...
fn inspect_harness_backend_pin_drives_generated_profile() {
    let project = TestProject::new("runtime-inspect-backend-pin-drives-profile");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "async")]
...
fn sim_config_profiles_and_backend_knobs_drive_test_output() {
    let project = TestProject::new("runtime-sim-config-schema");
    project.write(
        "Kobo.toml",
        r#"[sim]
...
fn sim_config_seed_count_runs_scheduler_portfolio() {
    let project = TestProject::new("runtime-sim-config-seed-count");
    project.write(
        "Kobo.toml",
        r#"[sim]
default_profile = "quick"
...
fn loom_scheduler_choice_changes_executed_trace() {
    let project = TestProject::new("runtime-loom-scheduler-policy");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "sync")]
...
fn sim_backend_loom_max_branches_drives_exhaustive_budget() {
    let project = TestProject::new("runtime-sim-config-max-branches");
    project.write(
        "Kobo.toml",
        r#"[sim]
...
fn sim_backend_replay_token_none_suppresses_recorded_token_and_keeps_backend_version() {
    let project = TestProject::new("runtime-sim-config-replay-token-none");
    project.write(
        "Kobo.toml",
        r#"[sim.backend.loom]
enabled = true
...
fn sim_backend_checkpoint_replay_is_validated_by_replay() {
    let project = TestProject::new("runtime-sim-config-checkpoint-replay");
    project.write(
        "Kobo.toml",
        r#"[sim.backend.loom]
enabled = true
...
fn semantic_engine_does_not_claim_checkpoint_artifact_validation() {
    let project = TestProject::new("runtime-semantic-checkpoint-no-artifact-claim");
    project.write(
        "Kobo.toml",
        r#"[sim.backend.loom]
enabled = true
...
fn sim_config_default_profile_runs_without_cli_sim_flag() {
    let project = TestProject::new("runtime-sim-default-profile");
    project.write(
        "Kobo.toml",
        r#"[sim]
