...
fn integrated_service_handler_record_activity_spawn_local_and_parallel_are_visible() {
    let (project, file) = integration_project("service-integrated-inspect");
...
fn integrated_service_sim_witness_carries_full_product_loop_evidence() {
    let (project, file) = integration_project("service-integrated-witness");
    let output = run_kobo(
        &[
            s("test"),
            s("--sim"),
...
fn integrated_record_path_reuses_values_on_replay_and_changes_digest_when_mutated() {
    let (first_project, first_file) =
        record_replay_integration_project("service-integrated-record-replay-first", "region-a");
    ...
    let (second_project, second_file) =
        record_replay_integration_project("service-integrated-record-replay-second", "region-b");
...
fn integrated_activity_side_effect_is_not_rerun_by_deterministic_replay() {
    let (project, file) = integration_project("service-integrated-activity-replay-blocked");
    let output = run_kobo(
        &[
            s("test"),
            s("--sim"),
...
fn integrated_activity_policy_mutation_changes_witness_metadata() {
    let (base_project, base_file) = integration_project("service-integrated-activity-policy-base");
    ...
    let (mutated_project, mutated_file) = integration_project_with_activity_policy(
        "service-integrated-activity-policy-mutated",
        "region-a",
        "retry-with-jitter",
        "dedupe-key",
    );
...
fn integrated_negative_parallel_and_standalone_debt_are_gated() {
    let (project, _) = integration_project("service-integrated-negative-and-debt");
