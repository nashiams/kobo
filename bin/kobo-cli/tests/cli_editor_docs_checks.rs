...
fn complete_cli_surface_documents_required_commands() {
    let project = TestProject::new("model-cli-help");
...
fn inspect_clean_and_inspect_cargo_preserve_clean_rust_exit_ramp() {
    let project = TestProject::new("model-clean-exit-ramp");
...
fn lsp_stdio_links_current_witness_artifact_for_opened_file() {
    let project = TestProject::new("model-lsp-live-witness-links");
    let source = r#"
#[kobo::must_call(close)]
struct Token {}
...
#[test]
fn vscode_extension_check_exposes_syntax_and_actions() {
...
fn formatting_delegates_rust_shaped_code_to_rustfmt() {
    let rust_project = TestProject::new("model-fmt-rust-shaped");
    ...
    let ward_project = TestProject::new("model-fmt-ward-syntax");
    let ward_file =
