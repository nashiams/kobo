...
fn fuzz_runs_stateful_scenario_cases_on_top_of_scheduler_profile() {
    let project = TestProject::new("runtime-fuzz-stateful");
...
fn fuzz_harness_agreement_is_independent() {
    let project = TestProject::new("runtime-fuzz-independent");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "stateful-input")]
fn generated_queue_ops() {
...
fn concurrent_state_sugar_lowers_through_compiler_owned_generated_rust() {
    let project = TestProject::new("runtime-concurrent-sugar");
    let file = project.main_file(
        r#"
fn state_surface() {
    #[kobo::shared]
...
"#,
    ...
    assert_success(
        &output,
        "Slice 10 concurrent-state sugar should lower through build --emit-rust",
    );
...
fn k0062_guard_across_await_gives_lock_scope_refactoring_suggestion() {
    let project = TestProject::new("runtime-k0062-refactor");
    let file = project.main_file(
        r#"
...
fn live_cell_build_compiles_without_user_arc_swap_dependency() {
    let project = TestProject::new("runtime-live-cell-build");
    let file = project.main_file(
        r#"
fn main() {
    @live route: String = String::from("blue");
...
fn self_referential_struct_fix_proposes_box_restructure() {
    let project = TestProject::new("runtime-self-ref-fix");
    let file = project.main_file(
        r#"
struct Node {
    next: Node,
