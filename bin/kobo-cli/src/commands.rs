...
pub(crate) fn dispatch(command: KoboCommand) -> anyhow::Result<()> {
    match command {
        ...
        KoboCommand::Inspect { file, checked, strict, clean, erase_lifetimes, cargo } => {
            run::cmd_inspect(&file, resolve_cli_mode(checked, strict), clean, erase_lifetimes, cargo.as_deref())
        }
        KoboCommand::Dump { file } => run::cmd_dump(&file),
        KoboCommand::Perf { file, from, threshold } => {
            perf::cmd_perf(&file, from.as_deref(), threshold)
        }
        KoboCommand::Debt { file, json, summary, borrows, patterns, errors, watch } => {
            if watch {
        ...
        KoboCommand::Migrate { file, dry_run, apply, graph, root, actor } => {
            migrate::cmd_migrate(
                &file,
                dry_run,
                apply,
        ...
        KoboCommand::Bench { file, tick_budget } => {
            bench::cmd_bench_tick(&file, tick_budget)
        }
        KoboCommand::Watch { file, simple, build } => {
