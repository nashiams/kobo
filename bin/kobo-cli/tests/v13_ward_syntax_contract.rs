...
fn ward_syntax_desugars_to_attribute_model() {
    let ward_project = TestProject::new("v13-ward-desugar");
    let attr_project = TestProject::new("v13-ward-attr-equivalent");
...
fn attribute_ward_form_remains_supported() {
    let project = TestProject::new("v13-attribute-ward-supported");
...
fn ward_clean_rust_output_preserves_zero_kobo_dependency_exit_ramp() {
    let project = TestProject::new("v13-ward-clean-rust");
    let file = project.main_file(ward_source());
    let output = run_kobo(
