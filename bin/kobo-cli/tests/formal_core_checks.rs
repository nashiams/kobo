...
fn question_mark_lowers_to_error_exit_terminator() {
    let project = TestProject::new("model-core-question");
    let source = r#"
struct Fallible {}
...
fn panic_lowers_to_panic_terminator() {
    let project = TestProject::new("model-core-panic");
    let source = r#"
#[kobo::scenario(profile = "sync")]
...
fn return_lowers_to_return_terminator() {
    let project = TestProject::new("model-core-return");
    let source = r#"
#[kobo::scenario(profile = "sync")]
...
fn await_lowers_to_resume_and_cancel_edges() {
    let project = TestProject::new("model-core-await");
    let source = r#"
...
fn opaque_call_lowers_to_opaque_boundary_edge() {
    let project = TestProject::new("model-core-opaque");
    let source = r#"
...
fn source_spans_survive_kir_to_core_lowering() {
    let project = TestProject::new("model-core-source-spans");
    let source = r#"
struct Queue {}
struct Parcel {}
...
fn invalid_unmapped_core_node_fails_instead_of_inventing_span() {
    let project = TestProject::new("model-core-unmapped");
    let source = r#"
#[kobo::scenario(profile = "sync")]
fn unmapped_case() {
    return;
...
fn comments_strings_and_names_do_not_affect_core_obligations() {
    let project = TestProject::new("model-core-text-bait");
    let source = r#"
#[kobo::scenario(profile = "sync")]
fn text_bait_case() {
...
fn kwit_records_template_ids_versions_and_obligation_states() {
    let project = TestProject::new("model-core-template-evidence");
    let source = r#"
struct Queue {}
struct Parcel {}
...
fn summary_records_core_and_template_facts() {
    let project = TestProject::new("model-core-summary-evidence");
    let source = r#"
#[kobo::scenario(profile = "sync")]
fn summary_case() {
    return;
...
fn proof_seed_records_core_edge_and_obligation_exit_state() {
    let project = TestProject::new("model-core-proof-seed");
    let source = r#"
...
fn opaque_and_debt_boundaries_downgrade_exact_replay() {
    let project = TestProject::new("model-core-boundary-downgrade");
    let source = r#"
...
fn branch_terminator_records_distinct_core_targets() {
    let project = TestProject::new("model-core-branch-targets");
    let source = r#"
...
fn formal_core_is_compiler_owned_cfg_not_cli_text_scan() {
    let project = TestProject::new("model-core-compiler-owned-cfg");
    let source = r#"
#[kobo::must_call(done)]
struct Token {}
...
fn declared_ack_nack_requeue_is_not_guessed_as_queue_delivery() {
    let project = TestProject::new("model-core-declared-template");
    let source = r#"
#[kobo::must_call(ack | nack | requeue)]
struct ManualToken {}
