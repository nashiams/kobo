...
fn replay_critical_boundary_uses_imported_function_and_cargo_identity() {
    let project = TestProject::new("ecosystem-boundary-imported-function");
...
#[test]
fn lsp_boundary_actions_include_all_ecosystem_policy_choices() {
    let project = TestProject::new("ecosystem-boundary-lsp-actions");
    write_path_crate(&project, "payments");
    project.write(
        "Cargo.toml",
        &format!(
