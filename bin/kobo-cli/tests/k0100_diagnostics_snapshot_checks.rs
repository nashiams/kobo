...
fn assert_json_diagnostic_snapshot(name: &str, diagnostic: &Value) {
    ...
    insta::with_settings!({
        snapshot_path => "../../../tests/snapshots/k0100_diagnostics",
        sort_maps => true,
    }, {
...
fn k0101_liveness_escape_diagnostic_snapshot() {
    let project = TestProject::new("model-k0101-snapshot");
    let file = project.main_file(
        r#"
...
"#,
    ...
    assert_text_snapshot("k0101_liveness_escape_diagnostic", &output.combined());
...
fn k0104_replay_divergence_diagnostic_snapshot() {
    let project = TestProject::new("model-k0104-snapshot");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "async")]
fn stable_replay() {
...
"#,
    ...
    assert_json_diagnostic_snapshot("k0104_replay_divergence_diagnostic", &diagnostic);
    assert_contains(
        &replay.combined(),
        "expected",
...
fn k0105_budget_diagnostic_snapshot() {
    let project = TestProject::new("model-k0105-snapshot");
    ...
    assert_json_diagnostic_snapshot("k0105_budget_diagnostic", &diagnostic);
    assert_contains(&diagnostic.to_string(), "8", "K0105 should include budget");
}

#[test]
fn k0106_shrink_safety_diagnostic_snapshot() {
    let project = TestProject::new("model-k0106-snapshot");
    let file = project.main_file(
        r#"
#[kobo::scenario(profile = "async")]
fn shrink_replay() {
...
"#,
    ...
    assert_json_diagnostic_snapshot("k0106_shrink_safety_diagnostic", &diagnostic);
    assert_contains(
...
fn k0107_boundary_policy_diagnostic_snapshot() {
    let project = TestProject::new("model-k0107-snapshot");
    ...
    assert_json_diagnostic_snapshot("k0107_boundary_policy_diagnostic", &diagnostic);
...
fn k0108_trace_check_diagnostic_snapshot() {
    let project = TestProject::new("model-k0108-snapshot");
    let file = project.main_file(
        r#"
...
"#,
    ...
    assert_json_diagnostic_snapshot("k0108_trace_check_diagnostic", &diagnostic);
    assert_contains(
        diagnostic["explanation"].as_str().unwrap_or_default(),
        "Temporal checks",
