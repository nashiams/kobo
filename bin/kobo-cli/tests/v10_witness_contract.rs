...
fn emit_v10_witness(project: &TestProject) -> std::path::PathBuf {
...
#[test]
fn kwit_contains_v10_required_fields_and_shrink_metadata() {
    let project = TestProject::new("v10-witness-fields");
    ...
    for path in [
        ...
        assert_json_has_path(&witness, path, "v0.10 witness required field");
    }
...
fn unsafe_shrink_metadata_is_rejected_with_k0106() {
    let project = TestProject::new("v10-unsafe-shrink");
    let witness_path = emit_v10_witness(&project);
...
fn cancellation_hook_is_recorded_and_replayed_at_distinct_facade_points() {
    let task_first = TestProject::new("v10-cancel-task-first");
    let (task_path, task_witness) =
        emit_cancel_witness(&task_first, "ward.task();\n    ward.time.now();");
    let time_first = TestProject::new("v10-cancel-time-first");
