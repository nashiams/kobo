...
pub(crate) fn dispatch(command: KoboCommand) -> anyhow::Result<()> {
    match command {
        ...
        KoboCommand::Inspect { file, checked, strict, clean, cargo } => {
            run::cmd_inspect(&file, resolve_cli_mode(checked, strict), clean, cargo.as_deref())
        }
        KoboCommand::Dump { file } => run::cmd_dump(&file),
        KoboCommand::Perf { file, from, threshold } => {
            perf::cmd_perf(&file, from.as_deref(), threshold)
        }
        KoboCommand::Debt { file, json, summary, borrows, patterns, watch } => {
            if watch {
        ...
        KoboCommand::Watch { file, simple } => {
            watch::cmd_watch(&file, simple)
        }
    }
}