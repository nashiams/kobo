...
fn backend_registry_discloses_full_depth_for_compiler_owned_and_registered_boundaries() {
    let project = TestProject::new("ecosystem-backend-full-depth-scope");
...
#[test]
fn exact_witness_discloses_ecosystem_ecosystem_scope_without_full_exploration() {
    let project = TestProject::new("ecosystem-exact-witness-ecosystem-scope");
...
"#,
    ...
    assert_contains(
        ...
        "ecosystem witness should include explicit ecosystem boundary evidence",
    );
    assert_contains(
        &witness.to_string(),
...
fn activity_witness_carries_retry_idempotency_and_result_metadata() {
    let project = TestProject::new("ecosystem-activity-witness-metadata");
    project.write(
        "reqwest.kobo.d.toml",
        r#"schema_version = 0
...
fn test_sim_rejects_activity_boundary_without_complete_metadata() {
    let project = TestProject::new("ecosystem-activity-witness-incomplete-metadata");
    project.write(
        "reqwest.kobo.d.toml",
        r#"schema_version = 0
...
"#,
    ...
    assert_contains(
        &output.combined(),
        "K0125",
        "incomplete activity metadata should use the ecosystem activity diagnostic",
    );
    assert!(
...
fn exact_witness_preserves_repeated_record_calls_from_same_crate() {
    let project = TestProject::new("ecosystem-repeated-record-boundary-events");
    let file = project.main_file(
        r#"
#[kobo::boundary(crate = "reqwest", policy = "record", reason = "record gateway construction")]
use reqwest::Client;
...
fn record_boundary_captures_facade_call_arguments_and_return_payload() {
    let project = TestProject::new("ecosystem-record-boundary-real-io-arguments");
    let file = project.main_file(
        r#"
...
fn record_boundary_captures_external_method_call_arguments_and_return_payload() {
    let project = TestProject::new("ecosystem-record-boundary-method-io-arguments");
    let file = project.main_file(
        r#"
...
fn record_boundary_captures_external_method_call_inside_helper() {
    let project = TestProject::new("ecosystem-record-boundary-helper-method");
    let file = project.main_file(
        r#"
...
fn assert_free_function_boundary_policy(policy: &str, evidence: &str, event_kind: &str) {
    let project = TestProject::new(&format!("ecosystem-free-function-{policy}-boundary"));
    if policy == "model" {
...
fn assert_nested_free_function_boundary_policy(policy: &str, evidence: &str, event_kind: &str) {
    let project = TestProject::new(&format!("ecosystem-nested-free-function-{policy}-boundary"));
    if policy == "model" {
        write_valid_adapter_config(&project, "payments");
    }
    let file = project.main_file(&format!(
...
fn assert_imported_module_free_function_boundary_policy(
    ...
    let project = TestProject::new(&format!("ecosystem-module-free-function-{policy}-boundary"));
    if policy == "model" {
        write_valid_adapter_config(&project, "payments");
    }
    let file = project.main_file(&format!(
...
fn mutated_boundary_policy_breaks_exact_replay() {
    let project = TestProject::new("ecosystem-mutated-boundary-policy");
    let file = project.main_file(
        r#"
#[kobo::boundary(crate = "reqwest", policy = "record", reason = "record gateway construction")]
use reqwest::Client;
...
fn record_boundary_missing_recorded_event_breaks_exact_replay() {
    let project = TestProject::new("ecosystem-record-missing-evidence");
    let file = project.main_file(
        r#"
#[kobo::boundary(crate = "reqwest", policy = "record", reason = "record gateway construction")]
use reqwest::Client;
...
fn record_boundary_missing_io_capture_breaks_exact_replay() {
    let project = TestProject::new("ecosystem-record-missing-io-capture");
    let file = project.main_file(
        r#"
#[kobo::boundary(crate = "reqwest", policy = "record", reason = "record gateway construction")]
use reqwest::Client;
...
fn record_boundary_tampered_io_capture_hash_breaks_exact_replay() {
    let project = TestProject::new("ecosystem-record-tampered-io-capture");
    let file = project.main_file(
        r#"
#[kobo::boundary(crate = "reqwest", policy = "record", reason = "record gateway construction")]
use reqwest::Client;
...
fn record_boundary_tampered_ledger_io_capture_breaks_exact_replay() {
    let project = TestProject::new("ecosystem-record-tampered-ledger-io-capture");
    let file = project.main_file(
        r#"
#[kobo::boundary(crate = "reqwest", policy = "record", reason = "record gateway construction")]
use reqwest::Client;
...
fn record_boundary_event_evidence_is_call_specific() {
    let project = TestProject::new("ecosystem-record-call-specific-evidence");
    let file = project.main_file(
        r#"
#[kobo::boundary(crate = "reqwest", policy = "record", reason = "record gateway construction")]
use reqwest::Client;
...
fn opaque_boundary_witness_replays_as_partial_not_exact() {
    let project = TestProject::new("ecosystem-opaque-partial-replay");
    let file = project.main_file(
        r#"
...
fn configured_adapter_metadata_replays_in_canonical_boundary_evidence() {
    let project = TestProject::new("ecosystem-adapter-boundary-replay");
...
fn unvalidated_adapter_metadata_rejects_exact_witness_generation() {
    let project = TestProject::new("ecosystem-unvalidated-adapter-boundary");
    project.write(
        "Kobo.toml",
        r#"[ecosystem]
default = "opaque"
