...
use kobo_ir::{
    KoboSpan, ScenarioBoundary, ScenarioCoverageFacts, ScenarioModeledBoundary, ScenarioOp,
    ScenarioOpKind, ScenarioProgram,
};
use kobo_parser::KoboFile;
use syn::spanned::Spanned;
use syn::{
    Attribute, Block, Expr, ExprCall, ExprIf, ExprLit, ExprMatch, ExprMethodCall, ExprPath,
    ExprStruct, File, Item, ItemFn, Lit, Local, Macro, Member, Meta, Pat, PatIdent, PatType, Path,
    Stmt, UseTree,
};
...
pub fn build_scenario_program(
    ...
    let must_call_types = if artifacts.must_call_obligations.is_empty() {
        collect_must_call_types_from_attrs(file)
    } else {
        artifacts
    ...
    let mut lowerer = ScenarioLowerer {
        ...
        must_call_types,
        functions,
        use_crate_heads: collect_use_crate_heads_for_file(file),
        operations: Vec::new(),
    ...
    lowerer.operations.push(ScenarioOp {
        span: KoboSpan::generated(artifacts.file_id),
        kind: ScenarioOpKind::Return,
    });

    Ok(ScenarioProgram {
        file_id: artifacts.file_id,
...
fn collect_must_call_types_from_attrs(file: &File) -> HashMap<String, Vec<String>> {
    file.items
        .iter()
        .filter_map(|item| match item {
...
fn first_scenario(file: &File) -> Option<&ItemFn> {
...
impl<'a> ScenarioLowerer<'a> {
    ...
    fn record_known_call(
        ...
        if path_ends_with(&path.path, &["Client", "new"]) {
            ...
            self.operations.push(ScenarioOp {
                span: self.span(call),
                kind: ScenarioOpKind::ExternalBoundary { crate_name },
            });
            return true;
        }
        false
...
fn collect_use_crate_heads_for_file(file: &File) -> HashMap<String, String> {
    let mut output = HashMap::new();
    for item in &file.items {
        if let Item::Use(item_use) = item {
            collect_use_crate_heads(&item_use.tree, None, &mut output);
        }
    }
    output
}
