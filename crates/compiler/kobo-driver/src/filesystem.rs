...
/// Reads a `.kobo` source file, registers it in the session file set, and
/// returns the stable `FileId` together with the source text.
pub fn read_kobo_file(path: &Path, file_set: &mut FileSet) -> Result<(FileId, String), IoError> {
    let bytes = fs::read(path).map_err(|source| io_error_from(source, path))?;
    let source = String::from_utf8(bytes).map_err(|_| IoError::Utf8Error {
        path: path.to_path_buf(),
    })?;

    let file_id = file_set.register(path.to_path_buf(), source.clone());
    Ok((file_id, source))
}
...
mod tests {
    ...
    use super::{output_path_for, rs_path_for};
