...
fn missing_cancel_edge_rejected() {
    let project = TestProject::new("proof-tamper-cancel-edge");
...
fn cfg_edge_to_unknown_block_rejected_after_hash_recompute() {
    let project = TestProject::new("proof-tamper-cfg-transition");
...
#[test]
fn cfg_edge_to_known_block_with_invalid_state_rejected_after_hash_recompute() {
    let project = TestProject::new("proof-tamper-cfg-known-transition");
    let artifact_path = emit_artifact(
        &project,
        &sync_source("cfg_known_transition_case"),
        "cfg_known_transition_case",
    );

    rewrite_valid_certificate(&artifact_path, |value| {
        let first_edge = &mut value["core"]["cfg_edges"][0];
        first_edge["to"] = first_edge["from"].clone();
    });
...
fn async_model_material_changes_require_core_hash_update() {
    let project = TestProject::new("proof-tamper-async-core-hash");
    let artifact_path = emit_artifact(&project, async_source(), "async_cancel_case");
...
fn removed_future_state_obligation_rejected_after_hash_recompute() {
    let project = TestProject::new("proof-tamper-future-state-obligation");
    let artifact_path = emit_artifact(&project, async_source(), "async_cancel_case");
...
fn removed_discharge_rejected() {
    let project = TestProject::new("proof-tamper-discharge");
    let artifact_path = emit_artifact(
        &project,
...
fn stale_template_hash_rejected() {
    let project = TestProject::new("proof-tamper-template");
    let artifact_path = emit_artifact(
        &project,
...
fn unknown_boundary_policy_rejected() {
    let project = TestProject::new("proof-tamper-policy");
...
fn unknown_event_kind_rejected() {
    let project = TestProject::new("proof-tamper-event");
    let artifact_path = emit_artifact(
        &project,
...
fn unknown_unversioned_field_rejected() {
    let project = TestProject::new("proof-tamper-field");
    let artifact_path = emit_artifact(
        &project,
...
fn exact_over_disallowed_boundary_rejected() {
    let project = TestProject::new("proof-tamper-exact-boundary");
    let artifact_path = emit_artifact(&project, boundary_debt_source(), "debt_boundary_case");
...
fn exact_replay_with_coverage_loss_rejected() {
    let project = TestProject::new("proof-tamper-exact-coverage-loss");
    let artifact_path = emit_artifact(
        &project,
