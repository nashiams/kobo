...
#[test]
fn sim_init_rejects_unknown_profile_without_backend_default() {
    let project = TestProject::new("sim-init-unknown-profile");
    let file = project.copy_fixture("sim/gateway.kobo", "src/gateway.kobo");
    ...
    assert_failure(&output, "unknown sim profile should fail");
    let text = output.combined();
    assert_contains(
        &text,
    ...
    assert_contains(
        ...
        "failure should list stable profiles",
    );
    assert_not_contains(
        &text,
        "shuttle",
...
#[test]
fn sim_init_recommends_each_runtime_profile_from_target_shape() {
    let cases = vec![
        SimProfileCase {
            label: "sync",
...
fn inspect_sim_uses_policy_transparency_wording() {
    ...
    assert_not_contains(
        &text,
        "prototype",
        "inspect --sim must not expose prototype wording",
    );
    assert_not_contains(
        &text,
        &["v", "0."].concat(),
...
fn sim_scout_why_uses_stable_backend_recommendation_wording() {
    ...
    assert_not_contains(
        &text,
        &["v", "0."].concat(),
        "sim scout should not expose internal-staging product wording",
    );
}
...
fn backend_recommendations_avoid_version_stage_language() {
    ...
    assert_not_contains(
        &text,
        &["v", "0."].concat(),
        "backend recommendation output should not expose internal-staging product wording",
    );
    assert_not_contains(
        &text,
...
fn explain_reports_backend_profile_rationale() {
    ...
    assert_not_contains(
        &text,
        &["v", "0."].concat(),
        "profile explain should avoid internal-staging product wording",
    );
}
