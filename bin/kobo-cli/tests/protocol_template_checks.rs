...
fn run_witness(project: &TestProject, source: &str, target: &str) -> Value {
    ...
    assert_success(
        &output,
        "model evidence protocol-template fixture should run",
    );
...
fn queue_delivery_infers_ack_nack_requeue_without_manual_declaration() {
    let project = TestProject::new("model-template-queue");
    let source = r#"
struct Queue {}
struct Parcel {}
...
fn transaction_infers_commit_rollback_without_manual_declaration() {
    let project = TestProject::new("model-template-transaction");
    let source = r#"
struct Database {}
struct UnitOfWork {}
...
fn handler_reply_infers_reply_reject_cancel_without_manual_declaration() {
    let project = TestProject::new("model-template-handler");
    let source = r#"
struct RequestToken {}
...
fn spawned_task_infers_await_abort_detach_policy() {
    let project = TestProject::new("model-template-spawned-task");
    let source = r#"
#[kobo::scenario(profile = "async")]
async fn task_case() {
...
fn lock_permit_infers_release_or_safe_drop_boundary() {
    let project = TestProject::new("model-template-lock");
    let source = r#"
struct Gate {}
struct Permit {}
...
fn file_socket_infers_close_transfer_or_boundary() {
    let project = TestProject::new("model-template-file-socket");
    let source = r#"
struct Listener {}
struct Connection {}
...
fn standard_file_and_socket_apis_infer_file_socket_lifecycle() {
    let project = TestProject::new("model-template-std-file-socket");
    let source = r#"
#[kobo::scenario(profile = "network")]
...
fn template_facts_are_structured_data_not_diagnostic_text() {
    let project = TestProject::new("model-template-structured-data");
    let source = r#"
struct Queue {}
struct Parcel {}
...
fn unknown_helper_becomes_transfer_debt_or_escape_not_discharge() {
    let project = TestProject::new("model-template-helper-transfer");
    let source = r#"
struct Queue {}
struct Parcel {}
...
fn method_name_alone_does_not_infer_lifecycle_template() {
    let project = TestProject::new("model-template-no-method-name-only");
    let source = r#"
struct Inbox {}
struct PlainMessage {}
...
fn unrelated_method_name_does_not_discharge_obligation_token() {
    let project = TestProject::new("model-template-terminal-action-shape");
    let source = r#"
...
fn comments_strings_and_variable_names_do_not_create_or_discharge_obligations() {
    let project = TestProject::new("model-template-anti-gaming");
    let source = r#"
#[kobo::scenario(profile = "sync")]
fn anti_gaming_case() {
