...
fn sim_scout_fix_plan_covers_common_replay_boundary_classes() {
    let project = TestProject::new("ecosystem-fix-plan");
...
"#,
    ...
    let staged_version_field = ["\"version\":\"v", "0."].concat();
    assert!(
        !output.stdout.contains(&staged_version_field),
        "fix-plan JSON should not expose internal-staging version branding: {}",
        output.stdout
    );
    for expected in [
        "wall-clock",
...
"#,
        ...
        let project =
            TestProject::new(&format!("ecosystem-test-sim-replay-nondeterminism-{label}"));
...
fn sim_scout_fix_plan_ignores_comments_strings_and_local_same_name_types() {
    let project = TestProject::new("ecosystem-fix-plan-ast-evidence");
    let file = project.main_file(
        r#"
...
fn sim_scout_fix_plan_ignores_local_same_name_functions_modules_and_methods() {
    let project = TestProject::new("ecosystem-fix-plan-local-name-false-positives");
    let file = project.main_file(
        r#"
fn thread_rng() {}
...
fn sim_scout_fix_plan_resolves_block_local_scopes_before_classifying() {
    let project = TestProject::new("ecosystem-fix-plan-block-scope");
    let file = project.main_file(
        r#"
fn local_only() {
    mod sqlx {
...
fn sim_scout_fix_plan_resolves_block_local_external_imports() {
    let project = TestProject::new("ecosystem-fix-plan-block-imports");
    let file = project.main_file(
        r#"
fn replay_gap() {
    use rand::thread_rng;
...
fn sim_scout_fix_plan_uses_configured_registry_and_adapter_roots() {
    let project = TestProject::new("ecosystem-fix-plan-configured-ecosystem-roots");
    project.write(
        "Kobo.toml",
        r#"[ecosystem]
default = "opaque"
...
fn sim_scout_fix_plan_reports_invalid_ecosystem_config_instead_of_dropping_roots() {
    let project = TestProject::new("ecosystem-fix-plan-invalid-config");
    project.write(
        "Kobo.toml",
        r#"[ecosystem]
