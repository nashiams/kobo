...
fn inspect_must_expose_structured_solver_evidence_from_cli_path() {
    let case = FixtureCase::new("evidence-check-inspect", "tiered_mix.kobo");
...
fn solver_graph_fingerprint_must_change_when_cli_input_changes() {
    let base_case = FixtureCase::new("evidence-check-base", "tiered_mix.kobo");
    let variant_case = FixtureCase::new("evidence-check-variant", "tiered_mix.kobo");
...
fn migrate_dry_run_must_expose_solver_evidence_matching_inspect() {
    let case = FixtureCase::new("evidence-check-migrate", "tiered_mix.kobo");
...
fn migrate_solver_evidence_must_change_when_cli_input_changes() {
    let base_case = FixtureCase::new("evidence-check-migrate-base", "tiered_mix.kobo");
    let variant_case = FixtureCase::new("evidence-check-migrate-variant", "tiered_mix.kobo");
...
fn migrate_review_must_write_visible_decision_artifacts() {
    let case = FixtureCase::new("evidence-check-migration-ux", "tiered_mix.kobo");
...
fn migrate_must_report_loaded_persisted_decisions() {
    let case = FixtureCase::new("evidence-check-loaded-decisions", "tiered_mix.kobo");
...
fn migrate_uc_acceptance_fixtures_are_deterministic_and_visible() {
    for fixture_name in [
        ...
        let case = FixtureCase::new("evidence-check-uc-acceptance", fixture_name);
        let first = run_kobo(
...
fn solver_graph_and_migrate_output_are_stable_across_100_runs() {
    let case = FixtureCase::new("evidence-check-100-run", "tiered_mix.kobo");
...
fn bench_tick_budget_reports_measured_pipeline_timing() {
    let case = FixtureCase::new("evidence-check-tick-budget", "uc4_game_server.kobo");
...
fn check_reports_handler_request_state_leaks() {
    let case = FixtureCase::new("evidence-check-handler-leak", "handler_leak.kobo");
...
fn migrate_dry_run_reports_extract_before_borrow_fix() {
    let case = FixtureCase::new("evidence-check-s17-dry-run", "extract_before_borrow.kobo");
...
fn migrate_apply_writes_extract_before_borrow_fix() {
    let case = FixtureCase::new("evidence-check-s17-apply", "extract_before_borrow.kobo");
...
fn check_reports_live_borrow_liveness_diagnostic() {
    let case = FixtureCase::new(
        "evidence-check-live-borrow-liveness",
